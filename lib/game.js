(function(){
  ...
  Game.prototype.randomPosition = function(){
    var shipPos = this.startingShipPos();

    var x = Math.random() * Game.DIM_X;
    while (x < shipPos[0] + 50 && x > shipPos[0] - 50){
      x = Math.random() * Game.DIM_X;
    }

    var y = Math.random() * Game.DIM_Y;
    while (y < shipPos[1] + 50  && y > shipPos[1] - 50){
  ...
  Game.prototype.restart = function () {
    ...
    this.addAsteroids();
    this.ship = this.createShip();
    this.ship._gotHit = false;
    var currentScore = 0;

    this.$scoreBoard.data("num", currentScore);
    this.$scoreBoard.html("Asteroids Shot Down: " + currentScore);
  };
  ...
  Game.prototype.incrementScore = function(){
    var currentScore = this.$scoreBoard.data("num");
    currentScore++;
    this.$scoreBoard.data("num", currentScore);
    this.$scoreBoard.html("Asteroids Shot Down: " + currentScore);
  }
  ...
  Game.prototype.checkOver = function (ctx) {
    if (this.ship._gotHit === true){
      var text = 'You Lost! R to restart.';
      var textLine2 = '(only shot down ' + this.$scoreBoard.data("num") + ' Asteroids)';
      ctx.font = '50pt Arial';
      ...
      ctx.fillText(textLine2,
        Game.DIM_X / 2 - ctx.measureText(textLine2).width / 2,
        Game.DIM_Y / 2 + 70
      )
      return true;
